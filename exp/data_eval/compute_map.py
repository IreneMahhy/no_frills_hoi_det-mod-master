...
def match_hoi(pred_det, gt_dets):
    ...
    return is_match, remaining_gt_dets, score
...
def eval_hoi(hoi_id, global_ids, gt_dets, pred_dets_hdf5, out_dir):
    ...
    for global_id in global_ids:
        ...
        sorted_idx = [idx for idx, _ in sorted(
            ...
            reverse=True)]
        if num_dets > 0:
            max_score = hoi_dets[sorted_idx[0], 8]
        else:
            max_score = 0
        for i in sorted_idx:
            pred_det = {
        ...
        for i, gt_det in enumerate(candidate_gt_dets):
            is_match, pred_det_list, score = match_hoi(gt_det, pred_det_list)
            y_true.append(is_match)
            if is_match:
