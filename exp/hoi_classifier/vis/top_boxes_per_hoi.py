...
def select_best_boxes_across_dataset(
    ...
    for global_id in tqdm(pred_hois.keys()):
        human_obj_boxes_scores = pred_hois[global_id]['roles'][()]
        humans = pred_hois[global_id]['agents'][()]
        ...
        for i in range(has_role[0].size):
            hoi_id += 1
            action_i = has_role[0][i]
            role_i = has_role[1][i]
            all_cands = human_obj_boxes_scores[:, action_i, role_i, :]
            this_cand = np.where(all_cands[:, -1] != 0)
            for j in range(this_cand[0].size):
                global_id_det_id_score[hoi_id].append((global_id, j, all_cands[this_cand[0][j], -1],
                                                       all_cands[this_cand[0][j], :-1], humans[this_cand[0][j], :4]))
        '''
        for i in range(600):
    ...
    for hoi_id in tqdm(global_id_det_id_score.keys()):
        ...
        for i in range(exp_const.num_to_vis):
            global_id, det_id, score, object_boxes, human_boxes = global_id_det_id_score[hoi_id][i]
            global_ids.append(global_id)
            boxes_scores[i] = np.concatenate(human_boxes, object_boxes, score)
