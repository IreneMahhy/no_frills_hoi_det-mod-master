...
def _exp_detect_coco_objects(exp_const, data_const):
    prepare_data_for_faster_rcnn.prepare_data(exp_const, data_const)
...
def exp_detect_coco_objects_in_vcoco():
    exp_const = ExpConstants(exp_name='detect_coco_objects_in_vcoco',
                             out_base_dir=os.path.join(os.getcwd(), 'data_symlinks/vcoco_exp'))
    data_const = VcocoConstants()
    _exp_detect_coco_objects(exp_const, data_const)


def _exp_select_and_evaluate_confident_boxes(exp_const, data_const):
